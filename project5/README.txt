CSC 344 Lab 5: Markov Models with Context Functions
Ed Foley
March 17, 2014
...
Overview
========
This MATLAB code extends Lab 4's functionality to algorithmically generate MIDI files via Markov chains that are functions of other parameters. 

Song structure
==============
Currently, write_a_midi.m generates a song in an A–B–A-B pattern where all parts are the same length and A is written in the relative minor of B or vice-versa.
...
Context functions
=================
This version of the random music code allows state transition matrices to take on functions of other variables. These functions are re-evaluated each time the state transition matrix is referenced. The sample code, write_a_midi.m, uses this functionality to add "momentum" to the circle of fifths by making a chord change more likely to follow the circle based on how many consecutive chord changes have followed the circle.
...
5. A suppressed semicolon transfers that line's output to the terminal window. It will not cause a compile error.
