...
public:
    ...
    void renderNextBlock (AudioSampleBuffer& outputBuffer, int startSample, int numSamples)
    {
        if (angleDelta != 0.0)
        {
            if (tailOff > 0)
            {
                while (--numSamples >= 0)
                {
					// Next sample follows shape given in "waveform" method; amplitude modulated slowly
                    const float currentSample = (float) (waveform (currentAngle) * level * tailOff * sin(currentAngle / 1024.0));
					...
					// Decay factor; amplitude drops to 0.005 in about 6.0 seconds at 44100 samples/second
                    tailOff *= 0.99998;
                    
					// Note becomes sufficiently quiet to ignore
                    if (tailOff <= 0.005)
                    {
                        clearCurrentNote();
            ...
            else
            {
                while (--numSamples >= 0)
                {
                    const float currentSample = (float) (waveform (currentAngle) * level *  sin(currentAngle / 1024.0));
	...
	// Defines a half-square, half-sine wave
